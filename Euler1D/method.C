...
void createNumericalMethod(Method_ptr &numMethod, std::string scheme)
{
    ...
    else if (scheme == "MUSCL_HLLC")
    {
        numMethod = std::make_shared<MUSCL_HLLC>();
    }
    else if (scheme == "MUSCL_Godunov")
    {
        numMethod = std::make_shared<MUSCL_Godunov>();
    }
    else
    {
...
/* -------------------------------------------------------------------*/
/*                                                                    */
/*                    MUSCL-Hancock (HLLC) class                      */
/*                                                                    */
/* -------------------------------------------------------------------*/
MUSCL_HLLC::MUSCL_HLLC()
{
    m_name = "MUSCL_HLLC";
    m_ng = 2;
    limiter = "Minbee";
...
VectOfDouble MUSCL_HLLC::computeFlux(VectOfDouble &u_L, VectOfDouble &u_R, const double &dx, const double &dt, EoS_ptr &EoS)
{
    return m_HLLC.computeFlux(u_L, u_R, dx, dt, EoS);
}

void MUSCL_HLLC::getFullFlux(VectofVectDouble &F, VectofVectDouble &u, const double &dx, const double &dt, EoS_ptr &EoS)
{
    int size = u.size() - m_ng;
    VectofVectDouble uBarLUpdate(size, VectOfDouble(nVar));
    VectofVectDouble uBarRUpdate(size, VectOfDouble(nVar));
    ...
    for (int i = 0; i < size; i++)
    {
        uBarL[i] = m_SLIC.data_reconstruction(u[i], u[i + 1], u[i + 2], "L", limiter);
        uBarR[i] = m_SLIC.data_reconstruction(u[i], u[i + 1], u[i + 2], "R", limiter);
    };
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        for (int j = 0; j < nVar; j++)
        {
            uBarLUpdate[i][j] = uBarL[i][j] - 0.5 * dt / dx * (m_SLIC.calc_F(uBarR[i], wBarR)[j] - m_SLIC.calc_F(uBarL[i], wBarL)[j]);
            uBarRUpdate[i][j] = uBarR[i][j] - 0.5 * dt / dx * (m_SLIC.calc_F(uBarR[i], wBarR)[j] - m_SLIC.calc_F(uBarL[i], wBarL)[j]);
        };
    };
...
/* -------------------------------------------------------------------*/
/*                                                                    */
/*                  MUSCL-Hancock (Godunov) class                     */
/*                                                                    */
/* -------------------------------------------------------------------*/

MUSCL_Godunov::MUSCL_Godunov()
{
    m_name = "MUSCL_Godunov";
    m_ng = 2;
    limiter = "Minbee";
}

VectOfDouble MUSCL_Godunov::computeFlux(VectOfDouble &u_L, VectOfDouble &u_R, const double &dx, const double &dt, EoS_ptr &EoS)
{
    return m_Godunov.computeFlux(u_L, u_R, dx, dt, EoS);
}
