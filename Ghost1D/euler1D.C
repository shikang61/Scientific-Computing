...
double f(double p, VectOfDouble &w_L, VectOfDouble &w_R, EoS_ptr &EoS_1, EoS_ptr &EoS_2)
{
    return f_K(p, w_L, EoS_1) + f_K(p, w_R, EoS_2) + (w_R[V] - w_L[V]);
};

double df(double p, VectOfDouble &w_L, VectOfDouble &w_R, EoS_ptr &EoS_1, EoS_ptr &EoS_2)
{
    return df_K(p, w_L, EoS_1) + df_K(p, w_R, EoS_2);
};

double newtonRaphson(double initialGuess, double tolerance, int maxIterations, VectOfDouble &w_L, VectOfDouble &w_R, EoS_ptr &EoS_1, EoS_ptr &EoS_2)
{
    ...
    {
        f_p = f(p_prev, w_L, w_R, EoS_1, EoS_2);
        df_p = df(p_prev, w_L, w_R, EoS_1, EoS_2);
...
VectOfDouble raref(double S, VectOfDouble &w, std::string side, EoS_ptr &EoS)
{
    ...
    if (side == "L")
    {
        rho = w[RHO] * pow(2 / (gamma + 1) + (gamma - 1) / ((gamma + 1) * c_s) * (w[V] - S), 2 / (gamma - 1));
        u = 2 / (gamma + 1) * (c_s + 0.5 * (gamma - 1) * w[V] + S);
        p = (w[P] + p_ref) * pow(2 / (gamma + 1) + (gamma - 1) / ((gamma + 1) * c_s) * (w[V] - S), 2 * gamma / (gamma - 1));
    }
    else if (side == "R")
    {
        rho = w[RHO] * pow(2 / (gamma + 1) - (gamma - 1) / ((gamma + 1) * c_s) * (w[V] - S), 2 / (gamma - 1));
        u = 2 / (gamma + 1) * (-1 * c_s + 0.5 * (gamma - 1) * w[V] + S);
        p = (w[P] + p_ref) * pow(2 / (gamma + 1) - (gamma - 1) / ((gamma + 1) * c_s) * (w[V] - S), 2 * gamma / (gamma - 1));
    }
...
double S_shock(double p_star, VectOfDouble w, std::string side, EoS_ptr &EoS)
{
    ...
    if (side == "L")
    {
        return w[V] - c_s * sqrt((gamma + 1) / (2 * gamma) * (p_star + p_ref) / (w[P] + p_ref) + (gamma - 1) / (2 * gamma));
    }
    else if (side == "R")
    {
        return w[V] + c_s * sqrt((gamma + 1) / (2 * gamma) * (p_star + p_ref) / (w[P] + p_ref) + (gamma - 1) / (2 * gamma));
...
double S_T(VectOfDouble w_star, double p_star, std::string side, EoS_ptr &EoS)
{
    if (side == "L")
    {
        return (w_star[V] - c_s_star(p_star, w_star, EoS));
    }
    else if (side == "R")
    {
        return (w_star[V] + c_s_star(p_star, w_star, EoS));
...
VectOfDouble exactRiemannSolver(double x, double mid, double tStop, VectOfDouble &w_L_initial, VectOfDouble &w_R_initial, EoS_ptr &EoS_1, EoS_ptr &EoS_2)
{
    double initialGuess = 0.5 * (w_L_initial[P] + w_R_initial[P]);
    double p_star = newtonRaphson(initialGuess, 1e-6, 10e6, w_L_initial, w_R_initial, EoS_1, EoS_2);
    double u_star = 0.5 * (w_L_initial[V] + w_R_initial[V]) + 0.5 * (f_K(p_star, w_R_initial, EoS_2) - f_K(p_star, w_L_initial, EoS_1));
    ...
    if (S <= u_star)
    {
        ...
        { // Left shock wave
            double rho_L_star = rho_star_shock(p_star, w_L_initial, EoS_1);
            double S_L = S_shock(p_star, w_L_initial, "L", EoS_1);
        ...
        { // Left rarefaction wave
            double rho_L_star = rho_star_raref(p_star, w_L_initial, EoS_1);
            VectOfDouble w_Lfan = raref(S, w_L_initial, "L", EoS_1);
            VectOfDouble w_Lstar = {rho_L_star, u_star, p_star};
            double S_HL = S_H(w_L_initial, "L", EoS_1);
            double S_TL = S_T(w_Lstar, p_star, "L", EoS_1);
            if (S <= S_HL)
            {
                w_LEFT = w_L_initial;
            }
    ...
    else if (S >= u_star)
    {
        ...
        { // Right shock wave
            double rho_R_star = rho_star_shock(p_star, w_R_initial, EoS_2);
            double S_R = S_shock(p_star, w_R_initial, "R", EoS_2);
        ...
        { // Right rarefaction wave
            VectOfDouble w_Rfan = raref(S, w_R_initial, "R", EoS_2);
            VectOfDouble w_Rstar = {rho_star_raref(p_star, w_R_initial, EoS_2), u_star, p_star};
            double S_HR = S_H(w_R_initial, "R", EoS_2);
            double S_TR = S_T(w_Rstar, p_star, "R", EoS_2);
            if (S >= S_HR)
            {
                w_RIGHT = w_R_initial;
            }
...
std::pair<double, double> wave_speed_estimate(VectOfDouble &u_L, VectOfDouble &u_R, EoS_ptr &EoS)
{
    ...
    double p_star = newtonRaphson(initialGuess, 1e-6, 10e6, w_L, w_R, EoS, EoS);
